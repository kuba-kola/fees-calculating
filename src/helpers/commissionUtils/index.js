...
const calculateResult = (transactions, config) => {
  ...
  transactions.forEach((transaction) => {
    const { type, operation: { amount }, user_type: userType } = transaction;
    const isCashIn = type === 'cash_in';
    const isCashOutJuridical = type === 'cash_out' && userType === 'juridical';
    const isCashOutNatural = type === 'cash_out' && userType === 'natural';
    ...
    if (isCashIn) {
      ...
      if (weeklyAmount + transaction.operation.amount > cashOutNatural.week_limit.amount) {
        if (userWeeklyAmounts[userId].exceededLimit) {
          const commission = (
            calculateCommissions(transaction.operation.amount, cashOutNatural.percents)
          );
          commissionAmounts.push(commission);
        } else {
          const exceededAmount = (
            weeklyAmount + transaction.operation.amount - cashOutNatural.week_limit.amount
          );
